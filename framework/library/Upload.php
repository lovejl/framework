...
class Upload
{
	public $max_size = '';//set file size
	
	public $file_name = '';//file name 
	
	public $allow_types = '';//file allow types,use | split
	
	public $errmsg = '';//error message
	
	public $uploaded = '';//after upload file name
	
	public $save_path = '';//file save path
	
	private $files;//upload file
	 ...
	 * UPLOAD INIT
	 */
	public function init($save_path = '',$file_name = '',$allow_types = '', $max_size = '')
	{ 
		$this->file_name = empty($file_name) ? time() : $file_name;
		$this->save_path = empty($save_path) ? APP_PATH . '/upload/' : $save_path; 
		$this->allow_types = empty($allow_types) ? 'jpg|gif|png|zip|rar' : $allow_types;
		$this->max_size = empty($max_size) ? 5242880 : (int)$max_size;//default 5M
	}
	...
	{ 
		...
		{ 
			case 0 :
				...
				break; 
			case 1 :
				$this->errmsg = 'max than than php.ini set'; 
				break; 
			case 2 :
				$this->errmsg = 'max file size'; 
				break; 
			case 3 :
				$this->errmsg = 'upload only a part'; 
				break; 
			case 4 :
				$this->errmsg = 'not found file'; 
				break; 
			case 5 :
				$this->errmsg = 'size 0'; 
				break; 
			default :
				$this->errmsg = 'upload file';
				break; 
		}
		...
		{ 
			$this->errmsg = 'file size more than ' . $this->max_size; 
			return FALSE; 
		} 
		if(!$this->set_save_path())
		{
			return FALSE;
		}
	 ...
	 * CHECK FILE TYPE
	 */
	public function check_file_type($filename, $tmp_name)
	{ 
		$ext = $this->get_file_type($filename);
		$this->ext = strtolower($ext);
		$length = strlen($this->getTypeList($ext));
		$file = fopen($tmp_name, "rb");
		$bin = fread($file, $length);
		fclose($file);
		$fileHead = @unpack("H{$length}", $bin);
		if(strtolower($this->getTypeList($ext)) != $fileHead[1])
		{
			$this->errmsg = 'file ext error';
			return FALSE;
		}
		$allow_types = array_map('strtolower', explode('|', $this->allow_types));
		...
		{ 
			$this->errmsg = $this->ext . ' not allow'; 
			return FALSE; 
		} 
	} 
	...
	{ 
		...
		{
			$result = mkdir($this->save_path, 0777, true);
			if(!$result)
			{
				$this->errmsg = 'mkdir ' . $this->save_path . ' error';
				return FALSE;
			}
		}
		return TRUE;
